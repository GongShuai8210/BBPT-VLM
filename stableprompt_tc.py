...
def parser_args():
    ...
    parser.add_argument('--target_model',type=str,default='google/gemma-2b-it') #google/gemma-1.1-7b-it google/gemma-2b-it
    parser.add_argument('--agent_model',type=str,default='google/gemma-2b-it')
    # parser.add_argument('--target_model', type=str, default='FacebookAI/roberta-large')
    # parser.add_argument('--agent_model', type=str, default='google/gemma-2b-it')
    # parser.add_argument('--target_model',type=str,default='Qwen/Qwen2.5-3B-Instruct')
    # parser.add_argument('--agent_model',type=str,default='Qwen/Qwen2.5-3B-Instruct')
    ...
    parser.add_argument('--cache_dir',type=str,default='/home/gongshuai/code/LLMs/')
    ...
    #                         ''', )
    parser.add_argument('--prompt_per_example',type=int,default=4) #  wei mei ge yangben sheng cheng 4 ge tishi
    parser.add_argument('--update_term',type=int,default=5)
...
def main():
    ...
    generation_kwargs = {
    "top_k": 0.0, #禁用Tok-k 采样 0意味着，不限制候选词，所有词都可能被选中
    "top_p": 1.0, # 禁用 Nucleus采样 1意味着，所有词都有可能被选中
    "do_sample": True, # 随机采样 False每次只选择概率最高的
    "pad_token_id": agent_tokenizer.eos_token_id,
    "max_new_tokens":args.max_prompt_length,
    "min_length": -1,#无最小长度限制
    }
        ...
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                query_encoded = agent_tokenizer.apply_chat_template(
                    query_text,
                    return_tensors='pt'
                ).view(-1).to(device) #apply_chat_template自动添加特殊字符
                
                response_tensors = ppo_trainer.generate(
                    query_encoded,
                    **generation_kwargs,
                    return_prompt=False, #表示只返回生成的新内容，不包含输入部分
