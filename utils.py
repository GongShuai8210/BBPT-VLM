...
class TopAccuracyTextsNoDuplicates:
    def __init__(self, max_size=5):
        self.heap = []
        self.text_map = {}  # 以文本为键，以（堆内位置，生成时间）为值的字典
...
def evaluation_soft(prompts,
    ...
    def _get_logits(texts, tokenizer, model, device):
        batch_size = len(texts)
        encoded_inputs = tokenizer(texts, padding='longest', truncation=True, return_tensors="pt", add_special_tokens=True)
        token_logits = model(**encoded_inputs.to(device)).logits  # dim (batch_size, longest_seq_length, vocab_size)
        next_token_indices = _get_next_token_index(encoded_inputs['input_ids']) #####由于是左填充，这么做是没问题的
        out_logits = token_logits[range(batch_size), next_token_indices, :] #预测位置所有token的logits
        return out_logits
    ...
    for prompt in prompts:
        ...
        #Get reward 奖励函数似乎有bug 输入的all_logits的话,targets似乎应该是对应的verbalizer_ids
        reward = get_reward(all_logits, targets, Fail_coefficient=Fail_coefficient, Success_coefficient=Success_coefficient)
        mean_reward = reward.mean().cpu()
        rewards.append(mean_reward)
